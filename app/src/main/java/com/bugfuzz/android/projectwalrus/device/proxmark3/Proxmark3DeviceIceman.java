 ...
 * Modified 2021 Christian Herrmann
 *
...
public class Proxmark3DeviceIceman extends UsbSerialCardDevice<Proxmark3CommandNG>
    ...
    @Keep
    public Proxmark3DeviceIceman(Context context, UsbDevice usbDevice) throws IOException {
        super(context, usbDevice, context.getString(R.string.idle));

        send(new Proxmark3CommandNG(Proxmark3CommandNG.VERSION));
    }
    ...
    protected Pair<Proxmark3CommandNG, Integer> sliceIncoming(byte[] in) {
        ...
        return new Pair<>(Proxmark3CommandNG.fromBytes(in), Proxmark3CommandNG.getByteLength());
    }
    ...
    public void createReadCardDataOperation(final AppCompatActivity activity,
        ...
        if (cardDataClass == HIDCardData.class) {
            ((OnOperationCreatedCallback) activity).onOperationCreated(
                        new ReadHIDOperation(this), callbackId
            );
            ...
            dialog.getViewModel().getSelectedReadSteps().observeForever(
                    new Observer<List<MifareReadStep>>() {
                        ...
                        public void onChanged(@Nullable List<MifareReadStep> readSteps) {
                            ((OnOperationCreatedCallback) activity).onOperationCreated(
                                    new ReadMifareOperation(Proxmark3DeviceIceman.this, readSteps),
                                    callbackId);
                        }
                    });
        } else {
    ...
    public MifareCardData.Block readMifareBlock(int blockNumber, MifareCardData.Key key,
        ...
        Pair<Boolean, MifareCardData.Block> result = sendThenReceiveCommands(
                new Proxmark3CommandNG(
                        Proxmark3CommandNG.MIFARE_READBL,
                        (short)0,
                ...
                new ReceiveSink<Proxmark3CommandNG, Pair<Boolean, MifareCardData.Block>>() {
                    @Override
                    public Pair<Boolean, MifareCardData.Block> onReceived(Proxmark3CommandNG in) {
                        if (in.cmd != Proxmark3CommandNG.ACK) {
                            return null;
                        }

                        if ((in.oldargs[0] & 0xff) == 0) {
    ...
    public String getVersion() throws IOException {
        ...
        try {
            Proxmark3CommandNG version = sendThenReceiveCommands(
                    new Proxmark3CommandNG(Proxmark3CommandNG.VERSION),
    ...
    public TuneResult tune(boolean lf, boolean hf) throws IOException {
        ...
        try {
            long arg = 0;
            if (lf) {
                arg |= Proxmark3CommandNG.MEASURE_ANTENNA_TUNING_FLAG_TUNE_LF;
            }
            if (hf) {
                arg |= Proxmark3CommandNG.MEASURE_ANTENNA_TUNING_FLAG_TUNE_HF;
            }
            if (arg == 0) {
            ...
            Proxmark3CommandNG result = sendThenReceiveCommands(
                    new Proxmark3CommandNG(
                            Proxmark3CommandNG.MEASURE_ANTENNA_TUNING,
                            (short)0,
                            new long[]{arg, 0, 0},
                            true
                            ),
                    new CommandWaiter(Proxmark3CommandNG.MEASURED_ANTENNA_TUNING, DEFAULT_TIMEOUT));
            if (result == null) {
            ...
            return new TuneResult(
                    lf, hf,
                    lf ? lfVoltages : null,
                    lf ? (result.oldargs[0] & 0xffff) / 1e3f : null,
                    lf ? (result.oldargs[0] >>> 16) / 1e3f : null,
                    lf ? 12e6f / ((result.oldargs[2] & 0xffff) + 1) : null,
                    lf ? (result.oldargs[2] >>> 16) / 1e3f : null,
                    hf ? (result.oldargs[1] & 0xffff) / 1e3f : null);
        } finally {
            releaseAndSetStatus();
        }
    }
    ...
    private static class ReadHIDOperation extends ReadCardDataOperation {
        ...
        public void execute(Context context, final ShouldContinueCallback shouldContinueCallback,
            ...
            Proxmark3DeviceIceman proxmark3Device = (Proxmark3DeviceIceman) getCardDeviceOrThrow();
            ...
            try {
                ...
                try {
                    proxmark3Device.send(
                            new Proxmark3CommandNG(
                                    Proxmark3CommandNG.HID_DEMOD_FSK,
                                    (short)0,
                                    new long[]{0, 0, 0},
                                    true
                            )
                ...
                proxmark3Device.send(new Proxmark3CommandNG(Proxmark3CommandNG.VERSION));
            } finally {
        ...
        public void execute(Context context, ShouldContinueCallback shouldContinueCallback)
            ...
            try {
                ...
                if (!proxmark3Device.sendThenReceiveCommands(
                        new Proxmark3CommandNG(
                                Proxmark3CommandNG.HID_CLONE_TAG,
                                (short)0,
                                new long[]{
                        ...
                        new WatchdogReceiveSink<Proxmark3CommandNG, Boolean>(DEFAULT_TIMEOUT) {
                            @Override
                            public Boolean onReceived(Proxmark3CommandNG in) {
                                return in.cmd == Proxmark3CommandNG.DEBUG_PRINT_STRING
            ...
            try {
                ...
                while (shouldContinueCallback.shouldContinue()) {
                    ...
                    Proxmark3CommandNG result = proxmark3Device.sendThenReceiveCommands(
                            new Proxmark3CommandNG(
                                    Proxmark3CommandNG.READER_ISO_14443A,
                                    (short)0,
    ...
    private static class CommandWaiter
        ...
        CommandWaiter(short cmd, @SuppressWarnings("SameParameterValue") long timeout) {
            super(timeout);
